...
def fission_softmax_graph_transform(graph, a, matmul_inputs, softmax_axis, subblock_name):
    ...
    c0 = OGS.Node(
            ...
            name=subblock_name + "Fission_ReduceMax"
        )

    graph.nodes.append(c0)

    c1 = OGS.Node(
            op="Sub",
            inputs=np.append([prev_out], max_out),
            outputs=[sub_out],
            name=subblock_name + "Fission_Sub"
        )

    graph.nodes.append(c1)
    """
...
if __name__=="__main__":
    ...
    if SELECT_GRAPH == 0:
        orig_model_path = "../cases/candidate_cases/softmax_subgraph.onnx"
        fission_model_path = "../cases/onnx-export/op_fission/softmax_subgraph.onnx"
    elif SELECT_GRAPH == 1:
        orig_model_path = "../cases/cases_onnx/segformer_bs1_res512.onnx"
        fission_model_path = "../cases/cases_onnx/op_fiss_taso/segformer_bs1_res512_fission.onnx"
    elif SELECT_GRAPH == 2:
        orig_model_path = "../cases/cases_onnx/segformer_bs16_res512.onnx"
        fission_model_path = "../cases/onnx-export/op_fission/segformer_bs16_res512_fission.onnx"
    elif SELECT_GRAPH == 3:
        orig_model_path = "../cases/cases_onnx/candy.onnx"
        fission_model_path = "../cases/onnx-export/op_fission/candy_fission.onnx"
    elif SELECT_GRAPH == 4:
        orig_model_path = "../cases/cases_onnx/mobile_cases/mobilevit_image_classification_const_initializer.onnx"
        fission_model_path = "../cases/cases_onnx/mobile_cases/mobilevit_image_classification_const_initializer_fission.onnx"
    elif SELECT_GRAPH == 5:
        orig_model_path = "../cases/cases_onnx/mobile_cases/c3d.onnx"
        fission_model_path = "../cases/cases_onnx/mobile_cases/op_fiss_taso/c3d.onnx"
    else:
        sys.exit()
    """
    ...
    if SELECT_GRAPH == 0:
        ...
        a = torch.Tensor(np.random.rand(1, 65536, 32))
        b = torch.Tensor(np.random.rand(1, 1024, 32))
        c = torch.Tensor(np.random.rand(1, 1024, 32))

        # input tensors for softmax_subgraph.onnx
        runtime_inputs[onnx_model.graph.input[0].name] = a.numpy()
        runtime_inputs[onnx_model.graph.input[1].name] = b.numpy()
        runtime_inputs[onnx_model.graph.input[2].name] = c.numpy()
    
    elif SELECT_GRAPH in [1, 2, 3, 4]:
        # input for segformer
        a = torch.Tensor(np.random.rand(input_shape[0], input_shape[1], input_shape[2], input_shape[3]))
        # input tensors for segformer_bs1_res1024.onnx
        runtime_inputs[onnx_model.graph.input[0].name] = a.numpy()
        
        # Relaxing for candy case
        ERROR_MARGIN = 5e-4
        
    elif SELECT_GRAPH == 5:
        # input for segformer
        a = torch.Tensor(np.random.rand(input_shape[0], input_shape[1], input_shape[2], input_shape[3], input_shape[4]))
        # input tensors for segformer_bs1_res1024.onnx
        runtime_inputs[onnx_model.graph.input[0].name] = a.numpy()
    else:
        print("Unsupported SELECT_GRAPH case")
        sys.exit()
    """
    ...
    exit()
    #%%
    ort_sess_orig = ort.InferenceSession(orig_model_path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
    outputs_orig_onnx = ort_sess_orig.run(None, runtime_inputs)
    
    #%%
    ort_sess_fission = ort.InferenceSession(fission_model_path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
    outputs_fission_onnx = ort_sess_fission.run(None, runtime_inputs)

    #%%
    print("orig_graph vs fission_graph correctness: ", 
          np.all(torch.isclose(torch.Tensor(outputs_orig_onnx), torch.Tensor(outputs_fission_onnx), atol=ERROR_MARGIN).numpy()))
    # %%
    # finding max difference between tensor values (if required)
    # fission_linear = torch.Tensor(outputs_fission_onnx).view(-1)
    # orig_linear = torch.Tensor(outputs_orig_onnx).view(-1)
    # diff = []
    # for idx in range(len(orig_linear)):
    #     diff.append(abs(fission_linear[idx] - orig_linear[idx]))

    # np_diff = np.array(diff)
    
    # # %%
    # print("Max Diff: ", np.max(np_diff))
# %%
